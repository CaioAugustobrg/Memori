...
class EntityFact {
  ...
  public async create(
    ...
    for (let i = 0; i < facts.length; i++) {
      ...
      if (conversationId) {
        ...
        if (factRow.length > 0) {
          await this.conn.execute(
            `INSERT INTO memori_entity_fact_mention(uuid, entity_id, fact_id, conversation_id) VALUES ($1, $2, $3, $4) ON CONFLICT (entity_id, fact_id, conversation_id) DO NOTHING`,
  ...
  public async getFactsByIds(factIds: (string | number)[]): Promise<CandidateFactRow[]> {
    ...
    const factRows = await this.conn.execute(
      `SELECT id, content, date_created FROM memori_entity_fact WHERE id = ANY($1)`,
      [factIds]
    );
